...
func (c *oidcClient) startSync(ctx context.Context) {
	...
	go func() {
		c.client.SyncProviderConfig(ctx, c.connector.GetIssuerURL())
		close(c.firstSync)
		<-c.syncCtx.Done()
		// close(stop)
	}()
...
func claimsFromUserInfo(oidcClient *oidc.Client, issuerURL string, accessToken string) (jose.Claims, error) {
	...
	if err != nil {
		return nil, trace.NotFound("If the issuer URL is not HTTPS")
	}
	...
	if err != nil {
		return nil, trace.NotFound("UserInfo endpoint is not HTTPS")
	}
