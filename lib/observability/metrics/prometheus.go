...
// in prometheus unary and/or stream interceptors.
func CreateGRPCServerMetrics(latencyEnabled bool, labels prometheus.Labels) *grpcprom.ServerMetrics {
	...
	if latencyEnabled {
		...
		serverOpts = append(serverOpts, grpcprom.WithServerHandlingTimeHistogram(histOpts...))
	}
...
// in prometheus unary and/or stream interceptors.
func CreateGRPCClientMetrics(latencyEnabled bool, labels prometheus.Labels) *grpcprom.ClientMetrics {
	clientOpts := []grpcprom.ClientMetricsOption{
		grpcprom.WithClientCounterOptions(grpcprom.WithConstLabels(labels)),
	}
	if latencyEnabled {
		histOpts := grpcHistogramOpts(labels)
		clientOpts = append(clientOpts, grpcprom.WithClientHandlingTimeHistogram(histOpts...))
	}
