...
func parseDuration(s string) (Duration, error) {
	...
	if s == "" {
		return 0, trace.BadParameter("time: invalid duration %s", orig)
	}
	for s != "" {
		var (
		...
		if i == 0 {
			return 0, trace.BadParameter("time: missing unit in duration %s", orig)
		}
		u := s[:i]
		s = s[i:]
		unit, ok := unitMap[u]
		if !ok {
			return 0, trace.BadParameter("time: unknown unit %s in duration %s", u, orig)
		}
		if v > (1<<63-1)/unit {
			// overflow
			return 0, trace.BadParameter("time: invalid duration %s", orig)
